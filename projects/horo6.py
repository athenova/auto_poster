    ...
    def __init__(self, sign, tg_chat_id, vk_group_id):
        ...
        builder = PostBuilder(
            message_builder=ContentBuilder(
                ...
                prompt_builder=IdentityPromptBuilder(f"Напиши практику дня для знака '{sign}' на {tomorrow.strftime('%Y-%m-%d')}. Используй структуру: практика дня, как выполнять, эффект")
            )
        )
