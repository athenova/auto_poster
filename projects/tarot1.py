    ...
    def __init__(self, sign, tg_chat_id, vk_group_id):
        ...
        builder = PostBuilder(
            message_builder=ContentBuilder(
                ...
                prompt_builder=IdentityPromptBuilder(f"Составь тароскоп для знака '{sign}' на неделю {tomorrow.strftime('%Y-%m-%d')} - {tomorrow_plus_7.strftime('%Y-%m-%d')}. Используй структуру: Энергия недели, Вызов, Подсказка или ресурс. Используй смайлики. Используй смайлики. Не используй 'Ок','Конечно'")
            )
        )
