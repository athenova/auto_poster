    ...
    def __init__(self, sign, tg_chat_id, vk_group_id):
        ...
        builder = PostBuilder(
            message_builder=ContentBuilder(
                ...
                prompt_builder=IdentityPromptBuilder(f"Какое число дня для знака '{sign}' на {tomorrow.strftime('%Y-%m-%d')}. Используй следующую структуру: Число дня, почему, значение для знака зодиака")
            )
        )
        processor = TagAdder(['#гороскоп', '#таро', '#числодня', f"#{sign}"])
        posters = [
