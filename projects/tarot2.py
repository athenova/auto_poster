    ...
    def __init__(self, sign, tg_chat_id, vk_group_id):
        ...
        builder = PostBuilder(
            message_builder=ContentBuilder(
                ...
                prompt_builder=IdentityPromptBuilder(f"Дай совет дня для знака '{sign}' на {tomorrow.strftime('%Y-%m-%d')}. Используй следующую структуру: Карта дня, послание, совет, предупреждение. Используй смайлики. Не используй 'Ок','Конечно'")
            )
        )
        processor = TagAdder(['#таро', '#советдня', f"#{sign}"])
        posters = [
