    ...
    def __init__(self, sign, tg_chat_id, vk_group_id):
        ...
        builder = PostBuilder(
            message_builder=ContentBuilder(
                ...
                prompt_builder=IdentityPromptBuilder(f"Какая энергия дня для знака '{sign}' на {tomorrow.strftime('%Y-%m-%d')}. Используй следующую структуру: Карта дня, общая характеристика, энергия. Используй смайлики. Не используй 'Ок','Конечно'")
            )
        )
        processor = TagAdder(['#таро', '#энергиядня', f"#{sign}"])
        posters = [
